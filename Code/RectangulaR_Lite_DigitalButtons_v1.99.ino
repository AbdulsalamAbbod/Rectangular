...
**Digital Buttons Version, Use pulldown resistor**
ResetPin 8
UpButtonPin 9
DownButtonPin 10
OkButtonPin 11
...
byte y_receive = 12, x_receive = 6, _Code_Position = 0;
...
#ifdef CustomCode
// 0 ==> 5 files each file has size of 10 codes each code are 4 bytes size which is quite enough for IR codes
...
void setup() {
 ...
 byte mycode =0;
   // save code at eeprom (values stay even after powering RectangulaR off)
    ...
    EEPROM.get(800, MemoryAddressLocation);
...
// draw the page frame with title
    ...
    while(true) {
        ...
        if (IRrecv.decode(&results)) {
            //Receive...
            if(_Code_Position < 10 && (results.value) > 1) {
                code_bits[_Code_Position] = IRrecv.results.bits;

                display.fillRect(83, 0, 15, 10, SSD1306_BLACK);
                display.setCursor(85, 0);
                display.println(code_bits[_Code_Position], DEC);
                display.setCursor(94, 0);
                display.println(F(" bit"));

                digitalWrite(indicator, HIGH);
                ReceivedData[_Code_Position] = (results.value);
                display.setCursor(x_receive, y_receive);
                display.println(ReceivedData[_Code_Position], HEX);
                ResetSleepingTime();
                display.display();
                delay(50);
                digitalWrite(indicator, LOW);
...
void SendingPage() {
    ...
    while(true) {
        ...
        if(((CurrentTime-PreviousTime0) > 1800) && (HomeButton) && (IsSelected)) {
            ...
            MemoryAddressLocation>40?MemoryAddressLocation=0:MemoryAddressLocation+=4;
            EEPROM.put(800, MemoryAddressLocation);
            IsSelected = false;
...
void PrintTheReceivedCode() {
    ...
    for( byte Code_Position = 0; Code_Position < (sizeof(ReceivedData)/sizeof(ReceivedData[0])); Code_Position++) {
        if(Code_Position > 4 && Column == 1) {
            y_send = 12; x_send = 70; Column = 0;
        }

        display.setCursor(x_send, y_send);
        if(ReceivedData[Code_Position] > 0) {
         ...
         if(IsButtonPressed(OkButton) && ((CurrentTime-PreviousTime0) > 700)) {
            ...
            switch(CurrentMode) {
               ...
               if(IsSelected){
                ...
                case 1: // Clear cache in Ram except EEPROM
                for(byte position = 0; position < 10; position++) {
                    ReceivedData[position] = 0;
                    _Code_Position = 0; x_receive = 6; y_receive = 12;// reset position
                    code_bits[position] = 0;
                }
                Notification(1);
                break;
...
void ReadTheButtonValue() {
      ...
      OkButton = digitalRead(OkButtonPin);
}
